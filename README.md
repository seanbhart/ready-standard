...
The Ready Standard defines `.ready` bundles: file-based product trees for
...
record format. The standard treats `.ready` as the durable bundle and record
...
- Public docs are Mintlify-compatible source in this repository's `docs/`
  directory. Vendored product-local bundles omit `docs/`.
- [skills/](skills/) - portable Ready Product Leader Skill Bundle.
- [templates/](templates/) - project starter templates.
- [manifest.yaml](manifest.yaml) - bundle index for reusable resources.

This repository is project-agnostic. Ready Room is an app that reads and writes
`.ready` bundles; it is not the standard itself.

## Portable Bundle

Projects that want agents to edit a Ready tree deterministically should vendor
the portable bundle beside the tree at `ready/standard/`. The portable bundle
is:
...
standard-update task, followed by syncing the portable bundle and migrating the
